...
export default function Board() {
  ...
  return (
    <div className="page-shell board-page">
      ...
      {!!posts.length && <div className="board-toolbar"><div className="category-filters" aria-label="카테고리">{['all', 1, 2, 3, 4, 5, 6, 7].map((value) => <button type="button" className={category === value ? 'selected' : ''} aria-pressed={category === value} onClick={() => setCategory(value)} key={value}>{value === 'all' ? 'ALL' : value === 7 ? 'AI' : value === 6 ? '책' : String(value).padStart(2, '0')}</button>)}</div><div className="board-view-controls" aria-label="보기 밀도">{[3, 6, 10].map((count) => <button type="button" className={columns === count ? 'selected' : ''} aria-label={`한 행에 ${count}개 보기`} title={`한 행에 ${count}개 보기`} aria-pressed={columns === count} onClick={() => setColumns(count)} key={count}><span className={`density-icon density-${count}`} aria-hidden="true" /></button>)}</div></div>}
      <div className={`post-grid columns-${columns}`}>{visiblePosts.map((post) => <Link className="post-card" to={`/board/${post.id}`} key={post.id}><img src={thumbnailUrl(post.thumbnail_path)} alt={post.title} loading="lazy" /><div><p>{post.category ? `${post.category === 7 ? 'AI' : post.category === 6 ? '책' : `CATEGORY ${String(post.category).padStart(2, '0')}`} · ` : ''}{post.author_name}</p><h2>{post.title}</h2><span>{post.description}</span><time>{new Date(post.created_at).toLocaleDateString('ko-KR')}</time></div></Link>)}</div>
