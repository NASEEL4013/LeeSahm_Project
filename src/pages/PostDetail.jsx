...
export default function PostDetail() {
  ...
  useEffect(() => {
    ...
    supabase.from('compositions').select('*').eq('id', id).single().then(({ data, error }) => {
      ...
      setPost(data); setTitle(data.title); setDescription(data.description)
    })
  }, [id])

  async function save() {
    const { error } = await supabase.from('compositions').update({ title: title.trim(), description: description.trim(), updated_at: new Date().toISOString() }).eq('id', id)
    if (error) return setMessage('수정하지 못했어.')
    setPost({ ...post, title: title.trim(), description: description.trim() }); setEditing(false); setMessage('수정했어.')
  }
  ...
  return (
    <article className="post-detail">
      ...
      <div className="post-detail-grid"><div className="post-detail-image"><img src={image} alt={post.title} /></div><div className="post-detail-copy">
        <p className="eyebrow">{post.author_name} · {new Date(post.created_at).toLocaleDateString('ko-KR')}</p>
        {editing ? <><input className="post-title-input" maxLength="80" value={title} onChange={(event) => setTitle(event.target.value)} /><textarea maxLength="2000" value={description} onChange={(event) => setDescription(event.target.value)} /><button className="button button-dark" disabled={!title.trim() || !description.trim()} onClick={save}>수정 완료</button><button className="text-button" onClick={() => setEditing(false)}>취소</button></> : <><h1>{post.title}</h1><p className="post-description">{post.description}</p></>}
        <div className="used-artworks"><p>사용 작품</p><ul>{post.composition.layers.map((layer) => <li key={layer.artworkId}>{layer.title}</li>)}</ul></div>
        <button className="text-button" onClick={downloadMap}>재현용 조합 파일 다운로드</button>
        {(mine || admin) && !editing && <div className="owner-actions">{mine && <><Link to={`/compose/${post.id}`}>그림 조합 수정</Link><button onClick={() => setEditing(true)}>설명 수정</button></>}<button onClick={remove}>게시물 삭제</button></div>}
